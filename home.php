...
<?php
	...
	if(is_array($arrTables) && count($arrTables)){
		...
		foreach($tg as $tn => $tgroup){
			...
			/* hide current table inUMU Time Table Managment System? */
			$tChkHL = array_search($tn, array());
			/* allowUMU Time Table Managment System 'add new' for current table? */
			$tChkAHAN = array_search($tn, array());

			/*UMU Time Table Managment SystemShowCount for current table? */
			$count_badge = '';
			...
			?>
				...
				<?php } ?>
					...
					<?php if($tChkHL === false || $tChkHL === null){ /* if table is not set as hidden inUMU Time Table Managment System */ ?>
