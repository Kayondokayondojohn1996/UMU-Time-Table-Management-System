<?php
	...
	~~~~~~ LIST OF FUNCTIONS ~~~~~~
		...
		get_home_links($homeLinks, $default_classes, $tgroup) -- process $homeLinks array and return custom links forUMU Time Table Managment System. Applies $default_classes to links if links have classes defined, and filters links by $tgroup (using '*' matches all table_group values)
	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'Faculties' => array('Faculties', '', 'resources/table_icons/building.png', 'None'),
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'Faculties' => "`Faculties`.`id` as 'id', `Faculties`.`name` as 'name'",
			'departments' => "`departments`.`id` as 'id', `departments`.`name` as 'name', IF(    CHAR_LENGTH(`Faculties1`.`name`), CONCAT_WS('',   `Faculties1`.`name`), '') as 'school'",
			'class_time_table' => "`class_time_table`.`id` as 'id', `class_time_table`.`day` as 'day', TIME_FORMAT(`class_time_table`.`time_start`, '%r') as 'time_start', TIME_FORMAT(`class_time_table`.`time_end`, '%r') as 'time_end', `class_time_table`.`unit_code` as 'unit_code', `class_time_table`.`venue` as 'venue', IF(    CHAR_LENGTH(`Faculties1`.`name`), CONCAT_WS('',   `Faculties1`.`name`), '') as 'school', IF(    CHAR_LENGTH(`departments1`.`name`), CONCAT_WS('',   `departments1`.`name`), '') as 'department', `class_time_table`.`year_of_study` as 'year_of_study'",
			'exam_time_table' => "`exam_time_table`.`id` as 'id', if(`exam_time_table`.`date`,date_format(`exam_time_table`.`date`,'%m/%d/%Y'),'') as 'date', TIME_FORMAT(`exam_time_table`.`time_start`, '%r') as 'time_start', TIME_FORMAT(`exam_time_table`.`time_end`, '%r') as 'time_end', `exam_time_table`.`unit_code` as 'unit_code', `exam_time_table`.`venue` as 'venue', IF(    CHAR_LENGTH(`Faculties1`.`name`), CONCAT_WS('',   `Faculties1`.`name`), '') as 'school', IF(    CHAR_LENGTH(`departments1`.`name`), CONCAT_WS('',   `departments1`.`name`), '') as 'department', `exam_time_table`.`year_of_study` as 'year_of_study'",
			...
			'student_details' => "`student_details`.`id` as 'id', `student_details`.`full_name` as 'full_name', IF(    CHAR_LENGTH(`Faculties1`.`name`), CONCAT_WS('',   `Faculties1`.`name`), '') as 'school', IF(    CHAR_LENGTH(`departments1`.`name`), CONCAT_WS('',   `departments1`.`name`), '') as 'department', `student_details`.`year_of_study` as 'year_of_study', `student_details`.`reg_no` as 'reg_no'",
			'notices' => "`notices`.`id` as 'id', `notices`.`notice` as 'notice', IF(    CHAR_LENGTH(`Faculties1`.`name`), CONCAT_WS('',   `Faculties1`.`name`), '') as 'school', IF(    CHAR_LENGTH(`departments1`.`name`), CONCAT_WS('',   `departments1`.`name`), '') as 'department', `notices`.`year_of_study` as 'year_of_study', if(`notices`.`date`,date_format(`notices`.`date`,'%m/%d/%Y'),'') as 'date'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			'Faculties' => "`Faculties` ",
			'departments' => "`departments` LEFT JOIN `Faculties` as Faculties1 ON `Faculties1`.`id`=`departments`.`school` ",
			'class_time_table' => "`class_time_table` LEFT JOIN `Faculties` as Faculties1 ON `Faculties1`.`id`=`class_time_table`.`school` LEFT JOIN `departments` as departments1 ON `departments1`.`id`=`class_time_table`.`department` ",
			'exam_time_table' => "`exam_time_table` LEFT JOIN `Faculties` as Faculties1 ON `Faculties1`.`id`=`exam_time_table`.`school` LEFT JOIN `departments` as departments1 ON `departments1`.`id`=`exam_time_table`.`department` ",
			'personal_time_table' => "`personal_time_table` ",
			'student_details' => "`student_details` LEFT JOIN `Faculties` as Faculties1 ON `Faculties1`.`id`=`student_details`.`school` LEFT JOIN `departments` as departments1 ON `departments1`.`id`=`student_details`.`department` ",
			'notices' => "`notices` LEFT JOIN `Faculties` as Faculties1 ON `Faculties1`.`id`=`notices`.`school` LEFT JOIN `departments` as departments1 ON `departments1`.`id`=`notices`.`department` "
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('TTMS_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('TTMS_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['TTMS_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['TTMS_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> Student TimeTable Management</a>
			</div>
