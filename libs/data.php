...
function checkdetailsStudent(){
	...
	if ($group=="students") {
		...
		if ($countcheck==0) {
			# code...
			echo '<div class="alert alert-primary"><strong>Whats up '.$name.', Welcome to TTMS-your UMU Time Table Managment System.You need to provide your personal details to be able to get personalized content.Simply click <a href="student_details_view.php">HERE</a> to get started</strong></div>';
		}
	}
}
...
function getclassestoday(){
	...
	if ($countcheck==0) {
		...
		<b>UMU Time Table Managment System!! No class today!!</b></div>';
	}
...
function getexams(){
	...
	if ($countcheck==0) {
		...
		echo '<div class="alert alert-success text-center">
		<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
		<b>UMU Time Table Managment System!! No exams today!!</b></div>';
	}
...
function classttlink(){
	...
	$getschoolName=mysqli_query($con,"SELECT name sname FROM Faculties WHERE id='$storeSchool'");
